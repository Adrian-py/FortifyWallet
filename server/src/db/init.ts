...
async function initializeDatabase() {
  const INIT_TABLE_QUERY = [
    ...
    "CREATE TABLE IF NOT EXISTS accounts (account_id int AUTO_INCREMENT, username varchar(20) UNIQUE, email varchar(20), password varchar(100), role_id int, reports_to int, PRIMARY KEY(account_id), FOREIGN KEY(role_id) REFERENCES roles(role_id), FOREIGN KEY(reports_to) REFERENCES accounts(account_id))",
