...
async function initializeDatabase() {
  const INIT_TABLE_QUERY = [
    ...
    "CREATE TABLE IF NOT EXISTS users (user_id int AUTO_INCREMENT, username varchar(20) UNIQUE, email varchar(20), password varchar(100), role_id int, reports_to int, PRIMARY KEY(user_id), FOREIGN KEY(role_id) REFERENCES roles(role_id), FOREIGN KEY(reports_to) REFERENCES users(user_id))",
    "CREATE TABLE IF NOT EXISTS wallets (wallet_id int AUTO_INCREMENT, user_id int, pubkey varchar(100), address varchar(100), PRIMARY KEY(wallet_id), FOREIGN KEY(user_id) REFERENCES users(user_id))",
